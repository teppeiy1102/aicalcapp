...
class _MiniCalcSheetState extends State<_MiniCalcSheet> {
  ...
  Widget _buildCalcFormulaDisplay(double fontSize, bool isDark) {
    final textColor = isDark ? Colors.white : Colors.black;
    final valBg = Colors.white.withOpacity(0.06);
    final valBorder = Colors.white.withOpacity(0.12);
    ...
    for (int i = 0; i < _termValues.length; i++) {
      // 値ボックス（タップで編集）
      final valStr = _fmt(_termValues[i]);
      widgets.add(
        GestureDetector(
          onTap: () => _editTermValue(i),
          child: Container(
